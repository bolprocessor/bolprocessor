/* Polymetric.c (BP2 version 2.9.4) */
...
if(ShowMessages) ShowMessage(TRUE,wMessage,"Interpreting structureÉ");
...
if(level != 0) {	/* '{' and '}' not balanced */
	sprintf(Message,"Incorrect polymetric expression(s): Ô{Õ and Ô}Õ are not balanced. Can't proceed furtherÉ");
...
level = 0;
if(NeedZouleb > 0) {
	if(ShowMessages) ShowMessage(TRUE,wMessage,"Applying serial toolsÉ");
	do {
...
if(ShowMessages) ShowMessage(TRUE,wMessage,"Expanding polymetric expressionÉ");
...
for(i=ZERO; ; i+=2L) {
	...
	if(foundinit) {
		/* Insert Ô/1Õ speed marker because none has been found. Otherwise, the initialÉ */
		/* É section numbers might be wrongly interpreted */
		pos_init = i;
		(**pp_a)[i] = T0; (**pp_a)[i+1] = 11; /* Ô/Õ */
		i += 2;
...
ENTERMETRONOM:
		rep = AnswerWith("Set metronome toÉ","60",Message);
		if(rep == ABORT) {
			rep = Answer("Do you really want to abort this job",'N');
			if(rep == YES) {
				r = ABORT;
...
CHANGEQUANTIZE:
		...
		rep = AnswerWith("Set quantization toÉ",Message,Message);
		if(rep == ABORT) {

WANTABORT:
			rep = Answer("Do you want to abort this job (answer ÔnoÕ to get another option)",'Y');
			if(rep == YES) {
				r = ABORT;
				goto QUIT;
				}
...
OkShowExpand = TRUE;	/* OK to display prolongational gaps Ò_Ó */
if(nsymb > 500L || numberprolongations > 100 || ((Prod / firstscale) > 100)) {
	if(ShowMessages) {
		if(nsymb > 500L) {
			sprintf(Message,"Expanded polymetric expression would contain %ld symbolsÉ",(long)nsymb);
			FlashInfo(Message);
			}
...
if(ShowMessages && Beta && 0) {
	sprintf(Message,"Expanding polymetric expression [position %ul]É",(*p_pos));
	ShowMessage(TRUE,wMessage,Message);
	}
PleaseWait();
...
restart = FALSE;
if(period && comma) {
	sprintf(Message,"Error in polymetric expression.\rThe same expression contains both Ô¥Õ and a commaÉ");
	if(ScriptExecOn) Println(wTrace,Message);
	else Alert1(Message);
	result = ABORT;
	goto OUT;
...
if(speed > TokenLimit || (1./speed) > TokenLimit) {
	Alert1("Unexpected overflow in polymetric formula (case 14). You may send this item to the designersÉ");
...
for(i = (*p_pos); (m = (*p_b)[i]) != TEND || (*p_b)[i+1] != TEND; i+=2L) {
	...
	if(Check_ic(ic,p_maxic,a,pp_c) != OK) {
 	...
 	if(m == T0 && p == 24) {  /* Ô**Õ scale down */
		...
		while(m == T1);
		if(s != 0.) scale = 1. / s;
		else if(Beta) Alert1("Err. PolyExpand() after Ô**Õ. s = 0.");
		i -= 2;
		continue;
 		}
 	if(m == T0 && p == 25) {  /* Ô\Õ speed down */
 		s = 0.;
 		do {
 			i += 2;
 			m = (*p_b)[i];
 	...
 	if(m == T0 && p == 11 && !tempomark) {	/* Ô/Õ speed up */
 		tempomark = TRUE;
 		foundtokens = TRUE;
 	...
 	if(m == T1) {							/* digit */
 		...
 		if(tempomark) {						/* changing tempo */
			...
			if((*p_b)[i+2] == T0 && (*p_b)[i+3] == 11) {	/* Ô/Õ */
				continue;
				}
			/* process number ÔgÕ: it is a silence. */	
			}
		}
	tempomark = FALSE;
		...
		if((m == T4 || m == T6 || (m == T0 && p == 1))) {
			if(ShowMessages && (m != T4 || MaxVar > 0) && (ShowGraphic || !Improvize)) {
				switch(m) {
					case T4:
						sprintf(Message,
							"Variable Ô%sÕ was found (field level %ld)",*((*p_Var)[p]),
								(long)a);
						break;
					case T6:
						sprintf(Message,
							"Unreplaced wild card Ô?%ldÕ was found (field level %ld)",
								(long)p,(long)a);
						break;
					case T0:
						sprintf(Message,
							"Unreplaced wild card Ô?Õ was found (field level %ld)",
								(long)a);
						break;
					}
				ShowMessage(TRUE,wMessage,Message);
...
for(a=kk=0; a < k; a++) {
	...
	if(!toobigitem) {
		if((L=LCM(L,(*p_p)[a],&overflow)) < 1.) {
			Println(wTrace,"Unexpected overflow in polymetric formula (case 1). You may send this item to the designersÉ");
	...
	if((*ptr_fixtempo)[a]) {
		...
		if(overflow) TellComplex();
		if(isequal == ABORT) {
			Println(wTrace,"Unexpected overflow in polymetric formula (case 5). You may send this item to the designersÉ");
			result = FAILED; goto OUT;
			}
		if(fixlength && (isequal != TRUE)) {
			if(comma) sprintf(Message,"Conflicting field duration (field %ld)É\r",
				(long)(a+1L));
			else sprintf(Message,"Conflicting beat duration (beat %ld)É\r",
				(long)(a+1L));
		...
		if(sign < 0 || xp < 1.) {
			if((*ptr_fixtempo)[a0]) {
				...
				if((lcm=LCM(qmax,(*p_q)[a],&overflow)) < 1.) {
					Println(wTrace,"Unexpected overflow in polymetric formula (case 2). You may send this item to the designersÉ");
					result = ABORT; goto OUT;
					}
				if(overflow) TellComplex();
...
if(comma) {
	if(Add((*p_P),(*p_Q),pmax,qmax,&xp,&xq,&overflow) != OK) {
		Println(wTrace,"Unexpected overflow in polymetric formula (case 7). You may send this item to the designersÉ");
		result = ABORT; goto OUT;
		}
	if(overflow) TellComplex();
	if(xp > MaxFrac || xq > MaxFrac) {
	...
	if(Add((*p_P),(*p_Q),((double) kk * pmax),qmax,&xp,&xq,&overflow)
			!= OK) {
		Println(wTrace,"Unexpected overflow in polymetric formula (case 8). You may send this item to the designersÉ");
		result = ABORT; goto OUT;
		}
	if(overflow) TellComplex();
	if(xp > MaxFrac || xq > MaxFrac) {
...
if(!toobigitem) {
	...
	for(a=0; a < k; a++) {
		...
		if((M = LCM(M,(*p_q)[a] * (*p_pp)[a],&overflow)) < 1.) {
			Println(wTrace,"Unexpected overflow in polymetric formula (case 10). You may send this item to the designersÉ");
			result = ABORT;
			goto OUT;
			}
		if(overflow) {
...
if(!SaidTooComplex) {
	SaidTooComplex = TRUE;
	FlashInfo("Formula is too complex. Roundings are performedÉ");
	if(ShowMessages)
		ShowMessage(TRUE,wMessage,"Formula is too complex. Roundings are performedÉ");
	}
return(OK);
}
