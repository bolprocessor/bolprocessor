...
OPENINPUT:
if(gChosenInputID != 0) {
	err = OMSAddPort(appSignature,gChosenInputID,omsPortTypeInput2,(OMSReadHook2UPP)readHook,
		(long)LMGetCurrentA5(),&gInputPortRefNum);
	if(err) {
		Alert1("BP2 couldn't create MIDI input port using default settings. You will need to setup the input, modify Ô-se.startupÕ and restart");
...
TRY:
if(opening) {
	err = OMSOpenConnections('Bel0',gChosenInputID,1,&conn,FALSE);
	switch(err) {
		case noErr:
			sprintf(Message,"Opened MIDI input Ô%sÕ ID = %ld",
				OMSinputName,(long)gChosenInputID);
	...
	OMSCloseConnections('Bel0',gChosenInputID,1,&conn);
	gChosenInputID = 0;
