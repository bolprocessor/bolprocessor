...
for(i=ZERO,space=TRUE,start=TRUE,k=0,nitem=1,runningstatus=0,
	...
	if(!isdigit(c)) {
		ShowMessage(TRUE,wMessage,"By-passing non numeric charactersÉ");
...
if((p_b = (MIDIcode**) GiveSpace((Size)imax * sizeof(MIDIcode))) == NULL)
	return(ABORT);
Alert1("Click ÔOKÕ. Then play notes on MIDI keyboard. Click mouse to terminate");
...
if(strlen(name) > 0) {
	...
	for(i=0; i < strlen(name); i++) {
		...
		if(!isalnum(c)) {
			sprintf(Message,
		"Space or incorrect character in name.  Can't accept Ô%cÕ",c);
			Alert1(Message);
			goto TRY;
			}
		}
...
HideWindow(Window[wMessage]);
if((*p_im) < 2L) {
	Alert1("No data receivedÉ"); goto END;
	}
r = OK;
