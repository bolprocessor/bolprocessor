/* TimeSetFunctions.c (BP2 version 2.9.4) */ 
...
if((*p_choice1)[i] > 5) {
	sprintf(Message,"Error1 choice1[%ld]=%ld.\r",(long)i,(long)(*p_choice1)[i]);
...
if(TraceTimeSet) {
	...
	if(shift1 > 0) {
		sprintf(Message,"should spend %ld milliseconds, but no solution.\r",(long)shift1);
		Print(wTrace,Message);
		}
	else {
		sprintf(Message,"should save %ld milliseconds, but no solution.\r",(long)-shift1);
		Print(wTrace,Message);
		}
...
if(redo && ((*p_choice1)[i] > 1)) {
	...
	sol1 = Next_choice((*p_sol_set1)[i],nseq,i,i0,j,(*p_Ts)[i],(*p_ts1)[i],
																(*p_ts2)[i],shift1,1);
	if((*p_choice1)[i] > 5) {
		sprintf(Message,"Error2 choice1[%ld]=%ld.\r",
			(long)i,(long)(*p_choice1)[i]);
		if(Beta) Alert1(Message);
		}
	if(sol1 == -8) {
		sprintf(Message,"\rnseq=%ld i=%ld side=1 j=%ld choice1=%ld sol_set=",
...
if(TraceTimeSet) {
	...
	Print(wTrace,Message);
	if(shift > 0) {
		sprintf(Message,"must spend %ld milliseconds\r",(long)shift);
		}
	else {
		sprintf(Message,"must save %ld milliseconds\r",(long)-shift);
		}
	Print(wTrace,Message);
	}
...
if(StepTimeSet) {
	...
	if(sol_set[0]) Print(wTrace,"S)hift sound-object\r");
	if(sol_set[1]) Print(wTrace,"B)reak tempo\r");
	if(sol_set[2]) Print(wTrace,"T)runcate beginning\r");
	if(sol_set[3]) Print(wTrace,"T)runcate end\r");
	if(i > i0 && !AllSolTimeSet) Print(wTrace,"R)evise previous sound-object(s)\r");
...
QUEST3:
	...
	if(rep == 'Q') {
		Print(wTrace,"\rAborted...\r"); ShowSelect(CENTRE,wTrace);
		return(ABORT);
		}
	if(rep == 'R'&& !AllSolTimeSet) {
REVISE:
		Print(wTrace,"Revising previous object(s)...\r"); ShowSelect(CENTRE,wTrace);
		return(4);
		}
	if(sol_set[0] && rep == 'S') {
		Print(wTrace,"Shifting sound-object...\r"); ShowSelect(CENTRE,wTrace);
		return(0);
		}
	else {
		if((sol_set[1]) && rep == 'B') {
			Print(wTrace,"Breaking tempo...\r"); ShowSelect(CENTRE,wTrace);
			return(1);
			}
		else {
			if(sol_set[2] && rep == 'T') {
				Print(wTrace,"Truncating beginning...\r"); ShowSelect(CENTRE,wTrace);
				return(2);
				}
			else {
				if(sol_set[3] && rep == 'T') {
					Print(wTrace,"Truncating end...\r"); ShowSelect(CENTRE,wTrace);
					return(3);
					}
				else {
...
if(shift < ZERO) {
	...
	if(TraceTimeSet) {
		...
		Print(wTrace,Message);
		if(shiftmore < 0) {
			sprintf(Message,"forces continuity. Previous object(s) must spend %ld milliseconds\r",
				(long)-shiftmore);
			}
		else {
			sprintf(Message,"forces continuity. Previous object(s) must save %ld milliseconds\r",
				(long)shiftmore);
			}
		Print(wTrace,Message);
		}
...
if(shift > ZERO) {
	...
	if(TraceTimeSet) {
		...
		Print(wTrace,Message);
		if(shiftmore < 0) {
			sprintf(Message,"can't be covered. Previous object(s) must spend %ld milliseconds. Err. Alternate_correction1().\r",
				(long)-shiftmore);
			if(Beta) Alert1(Message);
			}
		else {
			sprintf(Message,"can't be covered. Previous object(s) must save %ld milliseconds\r",
				(long)shiftmore);
			}
		Print(wTrace,Message);
		}
