/* FillPhaseDiagram.c (BP2 version 2.9.4) */ 
...
//  -------------  Fill phase diagram ------------------
	...
	/* kobj = 0		empty object or prolongational gap Ò_Ó		*/
	/* kobj = 1		silence Ò-Ó							*/
		...
		if(foundconcatenation) {
			...
			if(m == T25 || m == T9 || p > 1 || objectduration > 1. || isMIDIcontinuous) {
				...
				if(toofast && !overstrike) {		/* Sequence is too fast, we'll store proper duration nevertheless */
					/* We'll get the symbolic duration excluding Ô&Õ concatenation */
					objectduration
		...
		case T0:
			switch(p) {
				...
				case 23:
					...
					if(nseq > (*p_nmax)) {
						if(Beta) Println(wTrace,"\rErr. FillPhaseDiagram(). nseq > (*p_nmax) after Ô}Õ");
...
NEWSEQUENCE:
			...
			/*		if((*p_deftxpandkey)[level] > -1)
				...
				case 18:				/* Ô&Õ following terminal or simple note */
	//				if(level >= Maxlevel) goto NEXTTOKEN;
					skipzeros = FALSE;
					inext = (*p_im)[nseq];
					classofinext = Class(inext);
					...
					if((oldm != T3 && oldm != T25) || oldp < 1 || id < 2L) {
						sprintf(Message,"Concatenation Ô&Õ should follow a sound-object or simple note. One of them is misplaced");
					...
					if(nseq >= Minconc) {
						if(Beta) {
							Alert1("Err. FillPhaseDiagram(). nseq >= Minconc after Ô&Õ");
							goto ENDDIAGRAM;
							}
						goto NEWSEQUENCE;
						}
...
if(imax > 0.) {
	...
	if(Beta) {
		if(CorrectionFactor < 0.95 || CorrectionFactor > 1.05) {
			sprintf(Message,"\rCorrection factor %.3f might be out of rangeÉ",CorrectionFactor);
			Println(wTrace,Message);
			ShowMessage(TRUE,wMessage,Message);
			}
		if(ShowMessages) {
...
if(Beta) {
	...
	if(i < 0 || i >= imax) {
		Println(wTrace,"Err. UpdateParameter(). i < 0 ||Êi >= imax");
		return(ABORT);
		}
	}
...
if(Beta) {
	...
	imax = MyGetHandleSize((Handle)((*p_contparameters)[level].values)) / sizeof(ContParameterSpecs);
	if(i < 0 || i >= imax) {
		Println(wTrace,"Err. IncrementParameter(). i < 0 ||Êi >= imax");
		return(ABORT);
		}
	}
...
if(!ToldSkipped) {
	ToldSkipped = TRUE;
	sprintf(Message,"Some objects have been skippedÉ");
	ShowMessage(TRUE,wMessage,Message);
	FlashInfo(Message);
	}
return(OK);
